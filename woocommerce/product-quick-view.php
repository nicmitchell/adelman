...
<div class="row-fluid product-info product">
								...
								<ul class="slides">
									...
									<?php if ($attachment_ids && etheme_get_option('quick_images') == 'slider'): ?>
										<?php foreach ($attachment_ids as $key => $value): ?>
											<li><img src="<?php echo etheme_get_image($value, $mainImageWidth, $mainImageHeight, $mainImageCrop); ?>" title="<?php $image_title ?>"></a></li>
										<?php endforeach ?>						
									<?php endif ?>
									</a>
								</ul>
	...
	<?php endif ?>
	<div class="<?php if (etheme_get_option('quick_images') != 'none'): ?>span4<?php else: ?>span12<?php endif;?> product_meta">
		<?php if (etheme_get_option('quick_product_name')): ?>
			<h3 class="product-name test-triggers"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
		<?php endif ?>
